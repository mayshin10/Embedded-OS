...
uint32_t Kernel_msgQ_size(KernelMsgQ_t Qname){
		...
		if(sMsgQ[Qname].rear >= sMsgQ[Qname].front)
				...
				return sMsgQ[Qname].front - sMsgQ[Qname].rear + 1 - MSG_Q_SIZE_BYTE;
}

uint32_t Kernel_msgQ_empty_space(KernelMsgQ_t Qname){

		if(Qname >= KernelMsgQ_Num)
				return false;
		
		return MSG_Q_SIZE_BYTE - Kernel_msgQ_size(Qname);
}
...
bool Kernel_msgQ_dequeue(KernelMsgQ_t Qname, uint8_t* our_data){
		...
		*our_data = sMsgQ[Qname].Queue[idx];
